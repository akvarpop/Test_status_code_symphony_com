"""
Тестуемо усі сторінки сайту:https://demo2.symphonysolutions.info/ на статус код 200
We test all pages of the site: https://demo2.symphonysolutions.info/ for status code 200
"""
"""
Для запуску усіх тестів в терміналі: pytest -v -s /Users/antongrunt/Desktop/project/Test_status_code_symphony_com/tests
Для запуску тестів в файл з збереженням файлу Allure: 
pytest -v -s /Users/antongrunt/Desktop/project/Test_status_code_symphony_com/tests/test_status_code_demo.py --alluredir=allure_ss_demo
Збгенерувати посилання на звіт по теста: allure serve allure_ss_demo
"""
import pytest
import requests
...
def test_url_redirect_to_https(url):
    ...
    assert response.url == SERVICE_URL_DEMO, GlobalErrorMassages.WRONG_URL_HTTPS.value
